    ...
    <div className={cn('flex', size === 'md' ? 'gap-1' : 'gap-0.5')}>
      ...
      <div className={cn('flex grow flex-col items-start gap-1 py-1', size === 'md' ? 'px-1' : 'px-0.5')}>
        <div className='flex items-center gap-1'>
          <div className='text-sm font-semibold text-text-primary break-words'>{message}</div>
          {customComponent}
        </div>
        {children && <div className='text-xs font-normal text-text-secondary'>
          {children}
        </div>
        }
      </div>
      {close
        && (
        <button 
            className="z-[1000] p-1 rounded-md hover:bg-black/5 transition-colors cursor-pointer"
            onClick={close}
        >
...
export const ToastProvider = ({
  ...
  // Default duration logic moved inside effect or use state setter logic
  ...
  }, [mounted, params]) // removed defaultDuring from dependency to fix eslint exhaustive-deps warning potentially
...
Toast.notify = ({
  ...
  if (typeof window === 'object') {
    ...
    toastHandler.clear = () => {
      if (holder) {
        // React 18 uses root.unmount()
        setTimeout(() => { // unmount should be async safe or just done
             root.unmount()
