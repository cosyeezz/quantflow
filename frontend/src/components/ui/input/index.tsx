...
export const inputVariants = cva(
  'w-full appearance-none border border-transparent bg-components-input-bg-normal py-[7px] text-components-input-text-filled caret-primary-600 outline-none placeholder:text-components-input-text-placeholder hover:border-components-input-border-hover hover:bg-components-input-bg-hover focus:border-components-input-border-active focus:bg-components-input-bg-active focus:shadow-xs transition-all duration-200',
  {
    variants: {
      size: {
        regular: 'px-3 rounded-md text-sm', // Approximation of radius-md system-sm-regular
        large: 'px-4 rounded-lg text-base', // Approximation of radius-lg system-md-regular
      },
    },
    defaultVariants: {
      size: 'regular',
...
const Input = React.forwardRef<HTMLInputElement, InputProps>(({
  ...
  const handleNumberChange: ChangeEventHandler<HTMLInputElement> = (e) => {
    // If value is controlled (props.value is provided) and is number 0, logic might differ, 
    // but here we deal with event target value
    if (String(value) === '0' && e.target.value.length > 1) { 
        // Logic from original code, slightly adjusted for type safety
  ...
  const handleNumberBlur: FocusEventHandler<HTMLInputElement> = (e) => {
    ...
    if (e.target.value !== formattedValue) {
      e.target.value = formattedValue
      // We need to trigger change event manually if we modify value on blur
      // But React SyntheticEvent is read-only for some properties. 
      // Original code did this, assume it works or just call onChange.
       const newEvent = {
        ...e,
        target: {
          ...e.target,
    ...
    <div className={cn('relative w-full', wrapperClassName)}>
      ...
      <input
        ...
        placeholder={placeholder ?? (showLeftIcon
          ? (t('common.operation.search') || 'Search')
          : (t('common.placeholder.input') || 'Input'))}
        value={value}
      ...
      {
        unit && (
          <div className='absolute right-2 top-1/2 -translate-y-1/2 text-xs text-text-tertiary'>
            {unit}
          </div>
        )
      }
